...
<div class="min-h-screen flex items-center justify-center p-4">
    ...
    <div class="max-w-4xl w-full bg-white rounded-3xl shadow-2xl overflow-hidden flex flex-col md:flex-row">
        ...
        <div class="md:w-1/2 p-12">
            ...
            <?php if (isset($_GET['registered'])): ?>
                <p class="text-green-600 mb-4">Registration successful! Please login.</p>
            <?php endif; ?>

            <form action="../Services/process_login.php" method="POST" class="space-y-5">

                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-1.5">Email</label>
                    <input type="email" name="email"
                        class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:ring-2 focus:ring-indigo-500"
                ...
                <button type="submit"
                    class="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-3.5 rounded-xl shadow-lg transition-all">
                    Se Connecter
                </button>
